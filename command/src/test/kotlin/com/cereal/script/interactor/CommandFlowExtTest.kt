...
class CommandFlowExtTest {
    ...
    @Test
    fun `withRetry should not retry on RuntimeException`() =
        runTest {
            // Given
            var attemptCount = 0
            val testFlow =
            ...
            coVerify { logRepository.info("Skip retrying 'test action' due to unrecoverable or restartable exception 'Test runtime exception'") }
        }

    @Test
    fun `withRetry should not retry on UnrecoverableException`() =
        runTest {
            // Given
            var attemptCount = 0
            val testFlow =
            ...
            assertEquals(1, attemptCount)
            assertTrue(result.isFailure)
            coVerify { logRepository.info("Skip retrying 'test action' due to unrecoverable or restartable exception 'Test unrecoverable exception'") }
        }

    @Test
    fun `withRetry should retry with linear backoff for first five attempts`() =
        runTest {
            // Given
            var attemptCount = 0
            val testFlow =
    ...
    @Test
    fun `withRetry should use exponential backoff after five attempts`() =
        runTest {
            // Given
            var attemptCount = 0
            val testFlow =
    ...
    @Test
    fun `withRetry should stop retrying after maximum attempts`() =
        runTest {
            // Given
            var attemptCount = 0
            val testFlow =
    ...
    @Test
    fun `withLogging should log start and completion`() =
        runTest {
            // Given
            val context = ChainContext()
            val testFlow: Flow<ChainContext> =
                flow {
                    emit(context)
                }
    ...
    @Test
    fun `withLogging should log error`() =
        runTest {
            // Given
    ...
    @Test
    fun `withLogging should not log CancellationException as error`() =
        runTest {
            // Given
