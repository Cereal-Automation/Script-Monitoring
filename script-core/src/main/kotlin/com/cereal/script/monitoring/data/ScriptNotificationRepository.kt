...
class ScriptNotificationRepository(private val preferenceComponent: PreferenceComponent, private val notificationComponent: NotificationComponent): NotificationRepository {

    override suspend fun setItemNotified(item: Item) {
        preferenceComponent.setBoolean(item.isNotifiedKey(), true)
    }

    override suspend fun isItemNotified(item: Item): Boolean {
        return preferenceComponent.getBoolean(item.isNotifiedKey()) ?: false
    }
...
private fun Item.isNotifiedKey(): String {
    return "item_notification_${id}"
}
