...
class PlayStoreApiClientCacheTest {
    ...
    @Test
    fun `should use cached version when available and valid`() = runTest {
        // Setup cache with valid data
        coEvery { mockPreferenceComponent.getString("cache_tgtg_app_version_value") } returns "23.5.1"
        coEvery { mockPreferenceComponent.getString("cache_tgtg_app_version_expiration") } returns "${System.currentTimeMillis() / 1000 + 3600}" // 1 hour in future
        
        val cacheManager = PreferenceCacheManager(mockPreferenceComponent)
        val client = PlayStoreApiClient(
    ...
    @Test
    fun `should return null when cache is expired`() = runTest {
        // Setup cache with expired data
        coEvery { mockPreferenceComponent.getString("cache_tgtg_app_version_value") } returns "23.5.1"
        coEvery { mockPreferenceComponent.getString("cache_tgtg_app_version_expiration") } returns "${System.currentTimeMillis() / 1000 - 3600}" // 1 hour in past
    ...
    @Test
    fun `should return null when no cache exists`() = runTest {
        // Setup no cache
