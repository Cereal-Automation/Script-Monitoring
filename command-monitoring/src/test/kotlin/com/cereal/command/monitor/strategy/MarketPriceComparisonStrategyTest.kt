...
class MarketPriceComparisonStrategyTest {
    ...
    @Test
    fun `shouldNotify returns null when no variants with styleId exist`() =
        runBlocking {
    ...
    @Test
    fun `shouldNotify returns null when no matching market item is found`() =
        runBlocking {
            // Prepare
    ...
    @Test
    fun `shouldNotify returns null when market price is equal to item price`() =
        runBlocking {
            // Prepare
    ...
    @Test
    fun `shouldNotify returns null when market price is lower than item price`() =
        runBlocking {
            // Prepare
            val itemPrice = BigDecimal("100.00")
    ...
    @Test
    fun `shouldNotify returns null when price difference is below threshold`() =
        runBlocking {
            // Prepare
    ...
    @Test
    fun `shouldNotify returns notification when price difference is above threshold`() =
        runBlocking {
            // Prepare
    ...
    @Test
    fun `shouldNotify returns notification for multiple variants with different prices`() =
        runBlocking {
            // Prepare
            val item =
                Item(
    ...
    private fun createItemWithStyleId(
        styleId: String,
        price: BigDecimal,
    ): Item {
        return Item(
